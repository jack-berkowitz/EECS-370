...
main(int argc, char **argv)
{
    ...
    for (state.pc = 0; state.pc < memory_len; state.pc++) {
        ...
        opcode = (state.mem[state.pc] & (0b111 << 22)) >> 22;

        //R_type
        if ((opcode == 0b000) || (opcode == 0b001)) {
            reg_a = (state.mem[state.pc] & (0b111 << 19)) >> 19;
            reg_b = (state.mem[state.pc] & (0b111 << 16)) >> 16; 
            reg_dest = (state.mem[state.pc] & (0b111));
            if (opcode == 0b000) {
        ...
        if ((opcode == 0b010) || (opcode == 0b011) || (opcode == 0b100)) {
            reg_a = (state.mem[state.pc] & (0b111 << 19)) >> 19;
            reg_b = (state.mem[state.pc] & (0b111 << 16)) >> 16;
            offset = convertNum(state.mem[state.pc] & (0xFFFF));
            if (opcode == 0b010) {
